...
{
    public class ScheduledTaskAppService : BasisAppService, IScheduledTaskAppService
    {
        ...
        public ScheduledTaskAppService(IFreeSqlRepository<ScheduledTaskDO> scheduledTaskRepository, IFreeSqlRepository<TaskExecutionLogDO> taskExecutionLogRepository, IJobControl jobControl)
        {
        ...
        public async Task UpdateAsync(Guid id, ScheduledTaskDto dto)
        {
            var entity = await _scheduledTaskRepository.Where(x => x.Id == id).FirstAsync() ?? throw new EntityNotFoundException(typeof(ScheduledTaskDO), id);
