...
import DictDataForm, { type ModalRef } from './-ItemForm';
import SmartTable from '@/components/SmartTable';
import useApp from 'antd/es/app/useApp';
import { Link } from '@tanstack/react-router';
import { createFileRoute } from '@tanstack/react-router';

export const Route = createFileRoute('/admin/data-dictionaries/$type')({
    component: DictDataList
});

function DictDataList() {
    const tableRef = useRef<SmartTableRef>(null);
    const modalRef = useRef<ModalRef>(null);
    const { message } = useApp();
    const columns: SmartTableColumnType<DictDataListDto>[] = [
        {
            title: '字典标签',
            dataIndex: 'label',
        },
        ...
        <>
            <SmartTable
                ...
                request={async (params) => {
                    const data = await getDictDataList({ ...params, dictType: type });
                    return data;
                }}
                searchItems={[
